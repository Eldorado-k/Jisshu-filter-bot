...
async def save_group(bot, message):
    ...
    if temp.ME in check:
        if (str(message.chat.id)).startswith("-100") and not await db.get_chat(message.chat.id):
            ...
            btn = [[
                InlineKeyboardButton('⚡️ Support ⚡️', url=USERNAME)
            ]]
            reply_markup=InlineKeyboardMarkup(btn)
            await bot.send_message(
                chat_id=message.chat.id,
                text=f"<b>☤ Merci de m'avoir ajouté dans {message.chat.title}\n\n🤖 N'oubliez pas de me donner les droits d'admin 🤖\n\n㊝ Si vous avez des questions, utilisez les boutons ci-dessous ㊜</b>",
...
async def leave_a_chat(bot, message):
    ...
    if len(message.command) == 1:
        return await message.reply('<b>Utilisation : `/leave -100******`</b>')
    if len(r) > 2:
    ...
    try:
        btn = [[
            InlineKeyboardButton('⚡️ Propriétaire ⚡️', url=USERNAME)
        ]]
        reply_markup=InlineKeyboardMarkup(btn)
        await bot.send_message(
            chat_id=chat,
            text=f'😞 Bonjour,\nMon propriétaire m\'a demandé de quitter ce groupe 😔\n\n🚫 Raison : <code>{reason}</code>\n\nSi vous souhaitez m\'ajouter à nouveau, contactez mon propriétaire 👇',
            reply_markup=reply_markup,
        )
        await bot.leave_chat(chat)
        await db.delete_chat(chat)
        await message.reply(f"<b>J'ai bien quitté le groupe - `{chat}`</b>")
    except Exception as e:
        await message.reply(f'<b>🚫 Erreur - `{e}`</b>')

@Client.on_message(filters.command('groups') & filters.user(ADMINS))
async def groups_list(bot, message):
    msg = await message.reply('<b>Recherche en cours...</b>')
    chats = await db.get_all_chats()
    out = "Groupes enregistrés dans la base de données :\n\n"
    count = 1
    async for chat in chats:
        chat_info = await bot.get_chat(chat['id'])
        members_count = chat_info.members_count if chat_info.members_count else "Inconnu"
        out += f"<b>{count}. Titre - `{chat['title']}`\nID - `{chat['id']}`\nMembres - `{members_count}`</b>"
        out += '\n\n'
        count += 1
    try:
        if count > 1:
            ...
            await msg.edit_text("<b>Aucun groupe trouvé</b>")
    except MessageTooLong:
        with open('chats.txt', 'w+') as outfile:
            outfile.write(out)
        await message.reply_document('chats.txt', caption="<b>Liste de tous les groupes</b>")
