...
async def generate_link(client, message):
    ...
    if len(command_text) < 2:
        await message.reply("Veuillez donnez du film que vous souhaitez.  Exemple: `/link game of thrones`")
        return
    movie_name = command_text[1].replace(" ", "-")
    link = f"https://telegram.me/Marsh_mello_bot?start=getfile-{movie_name}"
