...
async def pm_search(client, message):
 ...
 #   if user_id in ADMINS: return
    ...
    if await db.get_pm_search_status(bot_id):
        ...
        await message.reply_text("<b><i>Je ne fonctionne pas ici. Recherchez des films dans notre groupe de recherche de films.</i></b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 Groupe de recherche de films", url='https://t.me/ZFlixTeam')]]))
...
async def group_search(client, message):
    ...
    if message.chat.id == SUPPORT_GROUP :
                ...
                if total != 0:
                    ...
                    msg = await message.reply_text(script.SUPPORT_GRP_MOVIE_TEXT.format(message.from_user.mention(), total), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Recoit le fichier ici 😉' , url='https://t.me/ZFlixTeam')]]))
    ...
    if settings["auto_filter"]:
        ...
        if 'hindi' in message.text.lower() or 'tamil' in message.text.lower() or 'telugu' in message.text.lower() or 'malayalam' in message.text.lower() or 'kannada' in message.text.lower() or 'english' in message.text.lower() or 'gujarati' in message.text.lower(): 
            ...
            return await message.reply("<b>Les liens ne sont pas permis ici ❌🤞</b>")
...
async def refercall(bot, query):
    btn = [[
        InlineKeyboardButton('Lien d\'invitation', url=f'https://telegram.me/share/url?url=https://t.me/{bot.me.username}?start=reff_{query.from_user.id}&text=Bonjour%21%20Découvrez%20un%20bot%20avec%20une%20immense%20bibliothèque%20de%20films%20et%20séries%20illimités.%20%F0%9F%98%83'),
        InlineKeyboardButton(f'⏳ {referdb.get_refer_points(query.from_user.id)}', callback_data='ref_point'),
        InlineKeyboardButton('Retour', callback_data='close_data')
    ]]
    reply_markup = InlineKeyboardMarkup(btn)
    await bot.send_photo(
        chat_id=query.message.chat.id,
        photo="https://graph.org/file/1a2e64aee3d4d10edd930.jpg",
        caption=f'Voici votre lien de parrainage :\n\nhttps://t.me/{bot.me.username}?start=reff_{query.from_user.id}\n\nPartagez ce lien avec vos amis. Pour chaque inscription via votre lien, vous recevrez 10 points. Après 100 points, vous obtiendrez un abonnement premium d\'1 mois.',
...
async def next_page(bot, query):
    ...
    btn.insert(0,[
	InlineKeyboardButton("📥 𝗘𝗻𝘃𝗼𝘆𝗲𝗿 𝗧𝗼𝘂𝘁 📥", callback_data=batch_link),
        ])
    btn.insert(1, [
        InlineKeyboardButton("Qualité", callback_data=f"qualities#{key}#{offset}#{req}"),
        InlineKeyboardButton("Saison", callback_data=f"seasons#{key}#{offset}#{req}"),
        InlineKeyboardButton("Langue", callback_data=f"languages#{key}#{offset}#{req}")
    ])    
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.append([InlineKeyboardButton(text="⪻ Retour à la page principale", callback_data=f"next_{req}_{key}_{offset}")])
    await query.message.edit_text("<b>Quelle saison voulez-vous ? Choisissez ci-dessous ↓↓</b>", reply_markup=InlineKeyboardMarkup(btn))
    return
...
async def season_search(client: Client, query: CallbackQuery):
    ...
    if not files:
        ...
        if not files:
            await query.answer(f"Désolé, {season.title()} introuvable pour {search}", show_alert=1)
            return
  ...
  #  del_msg = f"\n\n<b>⚠️ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ <code>{get_readable_time(DELETE_TIME)}</code> ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs</b>" if settings["auto_delete"] else ''
    ...
    if n_offset == '':
        btn.append(
            [InlineKeyboardButton(text="🚸 Plus de pages 🚸", callback_data="buttons")]
        )
    elif n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⪻ Retour", callback_data=f"season_search#{season}#{key}#{offset-int(MAX_BTN)}#{orginal_offset}#{req}"),
             InlineKeyboardButton(f"{math.ceil(offset/int(MAX_BTN))+1}/{math.ceil(total/int(MAX_BTN))}", callback_data="pages")]
        )
    elif offset == 0:
        btn.append(
            [InlineKeyboardButton(f"{math.ceil(offset/int(MAX_BTN))+1}/{math.ceil(total/int(MAX_BTN))}", callback_data="pages"),
             InlineKeyboardButton("Suivant ⪼", callback_data=f"season_search#{season}#{key}#{n_offset}#{orginal_offset}#{req}")])
    else:
        btn.append(
            [InlineKeyboardButton("⪻ Retour", callback_data=f"season_search#{season}#{key}#{offset-int(MAX_BTN)}#{orginal_offset}#{req}"),
             InlineKeyboardButton(f"{math.ceil(offset/int(MAX_BTN))+1}/{math.ceil(total/int(MAX_BTN))}", callback_data="pages"),
    ...
    btn.append([
        InlineKeyboardButton(text="⪻ Retour à l'accueil", callback_data=f"next_{req}_{key}_{orginal_offset}")])
    await query.message.edit_text(cap + links + js_ads, disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML, reply_markup=InlineKeyboardMarkup(btn))
    return
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.append([InlineKeyboardButton(text="⪻ Retour à l'accueil", callback_data=f"next_{req}_{key}_{offset}")])
    await query.message.edit_text("<b>Choisissez l'année de recherche ci-dessous ↓↓</b>", reply_markup=InlineKeyboardMarkup(btn))
    return
...
async def year_search(client: Client, query: CallbackQuery):
    ...
    if not files:
        await query.answer(f"Aucun résultat trouvé pour l'année {year.title()} avec {search}", show_alert=True)
        return
    ...
    btn.insert(0,[
        InlineKeyboardButton("📥 Envoyer tous les fichiers 📥", callback_data=batch_link),
    ])
    btn.insert(1, [
        InlineKeyboardButton("Qualité", callback_data=f"qualities#{key}#{offset}#{req}"),
        InlineKeyboardButton("Saison", callback_data=f"seasons#{key}#{offset}#{req}"), 
    ...
    if n_offset == '':
        ...
        btn.append(
            [InlineKeyboardButton("⪻ Retour", callback_data=f"years_search#{year}#{key}#{offset-int(MAX_BTN)}#{orginal_offset}#{req}"),
             InlineKeyboardButton(f"{math.ceil(offset/int(MAX_BTN))+1}/{math.ceil(total/int(MAX_BTN))}", callback_data="pages")]
        )
    elif offset == 0:
        btn.append(
            [InlineKeyboardButton(f"{math.ceil(offset/int(MAX_BTN))+1}/{math.ceil(total/int(MAX_BTN))}", callback_data="pages"),
             InlineKeyboardButton("Suivant ⪼", callback_data=f"years_search#{year}#{key}#{n_offset}#{orginal_offset}#{req}")])
    else:
        btn.append(
            [InlineKeyboardButton("⪻ Retour", callback_data=f"years_search#{year}#{key}#{offset-int(MAX_BTN)}#{orginal_offset}#{req}"),
             InlineKeyboardButton(f"{math.ceil(offset/int(MAX_BTN))+1}/{math.ceil(total/int(MAX_BTN))}", callback_data="pages"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if AUTH_CHANNEL and not await is_req_subscribed(client, query):
            await query.answer("J'apprécie ton intelligence mais ne sois pas trop malin 😒\nRejoins d'abord notre chaîne de mises à jour 😒", show_alert=True)
            return
        ...
        files = files_[0]
        btn = [[
            InlineKeyboardButton('🎗️ Obtenir votre fichier 🎗️', url=f'https://t.me/{temp.U_NAME}?start=file_{chat_id}_{file_id}')
        ]]
        reply_markup = InlineKeyboardMarkup(btn)
        
        return await query.message.edit(
            text=f'<b>Merci d\'avoir rejoint notre chaîne 🔥😗\nObtenez votre fichier : {files.file_name[:20]}.. en cliquant sur le bouton ci-dessous ⚡\n\nMaintenu par : @BotZFlix</b>',
            reply_markup=reply_markup
        )
        ...
        if has_free_trial:
            ...
            await query.message.edit_text(
                text="Félicitations 🎉 Vous obtenez un essai gratuit de <u>5 minutes</u> !\n\nProfitez de notre service premium pendant 5 minutes. Pour souscrire, cliquez ci-dessous :",
                disable_web_page_preview=True,                  
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💸 Voir les offres premium 💸", callback_data='seeplans')]]))
            await client.send_message(LOG_CHANNEL, text=f"#ESSAI_GRATUIT_ACTIVE\n\n👤 Utilisateur - {query.from_user.mention}\n⚡ ID - {user_id}", disable_web_page_preview=True)
            return   
        ...
        btn = [[
            InlineKeyboardButton("Regarder en ligne", url=online),
        ...
        await query.answer("Plus de pages disponibles 😊", show_alert=True)

    elif query.data == "pages":
        await query.answer("Ceci est un bouton de pagination 😅")

    elif query.data.startswith("lang_art"):
        _, lang = query.data.split("#")
        await query.answer(f"Vous avez sélectionné {lang.title()} ⚡️", show_alert=True)
  
    elif query.data == "start":
        buttons = [[
                InlineKeyboardButton('☆ Ajouter à votre groupe ☆', url=f'http://t.me/{temp.U_NAME}?startgroup=start')
                ],[
        ...
        buttons = [[
            InlineKeyboardButton('☆📸 Envoyer capture d\'écran 📸☆', url=f'https://t.me/Kingcey')
        ],[
            InlineKeyboardButton('💎 Plan personnalisé 💎', callback_data='other')
        ],[
    ...
    #jisshu
       ...
       await query.message.edit_media(
        ...
        buttons = [[
            InlineKeyboardButton('☎️ Contacter le propriétaire', user_id = ADMINS[0])
        ],[
        ...
        await query.answer('Seul l\'administrateur du bot peut activer ✓ ou désactiver ✗ cette fonctionnalité.', show_alert=True)
    
    elif query.data == "features":
        buttons = [[
              InlineKeyboardButton('Commandes Admin', callback_data='admincmd'),
              InlineKeyboardButton('Image vers Lien', callback_data='telegraph'),
              ], [
              InlineKeyboardButton('F-Sub', callback_data='fsub'),
              InlineKeyboardButton('Configuration Groupe', callback_data='earn')
              ], [
        ...
        if query.from_user.id not in ADMINS:
            return await query.answer('Ceci n\'est pas pour toi !', show_alert=True)
    
        buttons = [[
              InlineKeyboardButton('⋞ Retour', callback_data='features'),
              InlineKeyboardButton('Suivant ⪼', callback_data='admincmd2'),
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        buttons = [[
              InlineKeyboardButton('⋞ Retour', callback_data='admincmd')]]
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        buttons = [[
            InlineKeyboardButton('⇆ Ajoutez-moi à vos groupes ⇆', url=f'http://t.me/{temp.U_NAME}?startgroup=start')],
            [InlineKeyboardButton('⋞ Retour', callback_data='features')]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await query.message.edit_text(
            ...
            reply_markup = InlineKeyboardMarkup(
                [[
                    ...
                    InlineKeyboardButton('Mes Développeurs 😎',callback_data='mydevelopers')
                ],[
        ...
        await query.answer("Rencontrez les créateurs de ce bot :\n\n👨‍💻 @BotZFlix\n👨‍💻 @Kingcey\n👨‍💻 @Kingcey\n👨‍💻 et encore @Kingcey\n\n❤️ Un grand merci pour avoir rendu ce bot génial !", show_alert=True)
 
    elif query.data == "Source":
        buttons = [[
            InlineKeyboardButton('Dépôt', url='https://BotZFlix')
        ],[
        ...
        btn = [[
                InlineKeyboardButton("📲 Contacter le propriétaire", user_id = ADMINS[0])
           ],[
                InlineKeyboardButton("⇋ Retour ⇋", callback_data="about")
              ]]
        reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
              ...
              InlineKeyboardButton('⋞ Accueil', callback_data='features'),
        ...
        if settings is not None:
            buttons = [[
                InlineKeyboardButton('Filtre Auto', callback_data=f'setgs#auto_filter#{settings["auto_filter"]}#{grp_id}'),
                ...
                InlineKeyboardButton('Vérif. Orthographe', callback_data=f'setgs#spell_check#{settings["spell_check"]}#{grp_id}'),
                ...
                InlineKeyboardButton('Suppr. Auto', callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{grp_id}'),
            ...
            await query.message.edit_text("<b>Une erreur s'est produite</b>")
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("✅ Accepter cette demande ✅", callback_data=f"accept#{user_id}#{msg_id}")
        ],[
            InlineKeyboardButton("🚫 Rejeter cette demande 🚫", callback_data=f"reject#{user_id}#{msg_id}")
        ]]
        try:
            ...
            await query.answer("⚠️ Vous n'êtes pas membre de ce channel, veuillez d'abord rejoindre", show_alert=True)
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("✗ Rejeté ✗", callback_data=f"rj_alert#{user_id}")
        ]]
        btn = [[
            InlineKeyboardButton("♻️ Voir statut ♻️", url=f"{query.message.link}")
        ]]
        st = await client.get_chat_member(chnl_id, userid)
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            user = await client.get_users(user_id)
            request = query.message.text
            await query.answer("Message envoyé au demandeur")
            await query.message.edit_text(f"<s>{request}</s>")
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>Désolé, votre demande a été rejetée 😶</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
                await client.send_message(SUPPORT_GROUP, text=f"<b>💥 Bonjour {user.mention},\n\nDésolé, votre demande a été rejetée 😶</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=int(msg_id))
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("😊 Déjà disponible 😊", callback_data=f"already_available#{user_id}#{msg_id}")
        ],[
            InlineKeyboardButton("‼️ Non disponible ‼️", callback_data=f"not_available#{user_id}#{msg_id}")
        ],[
            InlineKeyboardButton("🥵 Indiquer année/langue 🥵", callback_data=f"year#{user_id}#{msg_id}")
        ],[
            InlineKeyboardButton("🙃 Disponible dans 1h 🙃", callback_data=f"upload_in#{user_id}#{msg_id}")
        ],[
            InlineKeyboardButton("☇ Téléchargé ☇", callback_data=f"uploaded#{user_id}#{msg_id}")
        ]]
        try:
            st = await client.get_chat_member(chnl_id, userid)
            if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("🚫 Non disponible 🚫", callback_data=f"na_alert#{user_id}")
        ]]
        btn = [[
            InlineKeyboardButton("♻️ Voir statut ♻️", url=f"{query.message.link}")
        ]]
        ...
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>Désolé, votre demande n'est pas disponible 😢</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
                await client.send_message(SUPPORT_GROUP, text=f"<b>💥 Bonjour {user.mention},\n\nDésolé, votre demande n'est pas disponible 😢</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=int(msg_id))
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("🙂 Téléchargé 🙂", callback_data=f"ul_alert#{user_id}")
        ]]
        btn = [[
            InlineKeyboardButton("♻️ Voir le statut ♻️", url=f"{query.message.link}")
        ]]
        st = await client.get_chat_member(chnl_id, userid)
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            user = await client.get_users(user_id)
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>Votre demande a été téléchargée ☺️</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
                await client.send_message(SUPPORT_GROUP, text=f"<b>💥 Bonjour {user.mention},\n\nVotre demande a été téléchargée ☺️</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=int(msg_id))
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("🫤 Déjà disponible 🫤", callback_data=f"aa_alert#{user_id}")
        ]]
        btn = [[
            InlineKeyboardButton("♻️ Voir le statut ♻️", url=f"{query.message.link}")
        ]]
        ...
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>Votre demande est déjà disponible 😋</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
                await client.send_message(SUPPORT_GROUP, text=f"<b>💥 Bonjour {user.mention},\n\nVotre demande est déjà disponible 😋</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=int(msg_id))
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("😌 Téléchargement dans 1 heure 😌", callback_data=f"upload_alert#{user_id}")
        ]]
        btn = [[
            InlineKeyboardButton("♻️ Voir le statut ♻️", url=f"{query.message.link}")
        ]]
        ...
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>Votre demande sera téléchargée dans 1 heure 😁</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
                await client.send_message(SUPPORT_GROUP, text=f"<b>💥 Bonjour {user.mention},\n\nVotre demande sera téléchargée dans 1 heure 😁</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=int(msg_id))
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        buttons = [[
            InlineKeyboardButton("⚠️ Indiquez année et langue ⚠️", callback_data=f"yrs_alert#{user_id}")
        ]]
        btn = [[
            InlineKeyboardButton("♻️ Voir le statut ♻️", url=f"{query.message.link}")
        ]]
        ...
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>Merci d'indiquer l'année et la langue, je pourrai alors télécharger 😬</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
                await client.send_message(SUPPORT_GROUP, text=f"<b>💥 Bonjour {user.mention},\n\nMerci d'indiquer l'année et la langue, je pourrai alors télécharger 😬</b>", reply_markup=InlineKeyboardMarkup(btn), reply_to_message_id=int(msg_id))
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        if str(userid) in user_id:
            await query.answer("Désolé, votre demande a été rejetée", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        if str(userid) in user_id:
            await query.answer("Désolé, votre demande n'est pas disponible", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        if str(userid) in user_id:
            await query.answer("Votre demande a été téléchargée", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        if str(userid) in user_id:
            await query.answer("Votre demande est déjà disponible", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        if str(userid) in user_id:
            await query.answer("Votre demande sera téléchargée dans 1 heure 😁", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
        ...
        userid = query.from_user.id
        if str(userid) in user_id:
            await query.answer("Merci d'indiquer l'année et la langue, je pourrai alors télécharger 😬", show_alert=True)
        else:
            await query.answer(script.ALRT_TXT, show_alert=True)
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        chat_id = message.chat.id
        settings = await get_settings(chat_id)
        searching_msg = await msg.reply_text(f'🔎 Recherche en cours : {search}')
        files, offset, total_results = await get_search_results(search)
        await searching_msg.delete()
        if not files:
            if settings["spell_check"]:
                ai_sts = await msg.reply_text(f'Vérification orthographique...')
        ...
        message = msg.message.reply_to_message  # msg sera la requête de callback
    ...
    del_msg = f"\n\n<b>⚠️ Ce message sera supprimé automatiquement après <code>{get_readable_time(DELETE_TIME)}</code> pour éviter les problèmes de copyright</b>" if settings["auto_delete"] else ''
    links = ""
    if settings["link"]:
        btn = []
    ...
    if offset != "":
        if total_results >= MAX_BTN:
            ...
            btn.insert(1,[
                InlineKeyboardButton("🚸 Pas plus de pages 🚸", user_id=ADMINS[0])
            ])
    else:
        btn.insert(0,[
            InlineKeyboardButton("📥 Envoyer Tous les Fichiers 📥", callback_data=batch_link),
    ...
    if spoll:
        m = await msg.message.edit(f"<b><code>{search}</code> a été trouvé, veuillez patienter pour les fichiers 📫</b>")
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"1/{math.ceil(int(total_results) / int(MAX_BTN))}", callback_data="pages"),
             InlineKeyboardButton(text="Suivant ⪼", callback_data=f"next_{req}_{key}_{offset}")]
        )
        key = f"{message.chat.id}-{message.id}"
        BUTTONS[key] = search
        req = message.from_user.id if message.from_user else 0
    ...
    if imdb:
        ...
        cap = f"<b>📂 Voici ce que j'ai trouvé pour votre recherche : {search}</b>"
...
async def advantage_spell_chok(message):
    ...
    if not movies:
        ...
        button = [[
            InlineKeyboardButton("🔍 Vérifier l'orthographe sur Google 🔍", url=f"https://www.google.com/search?q={google}")
        ]]
